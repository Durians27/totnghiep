    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    // =======================================================
                    // 🔥 KIỂM TRA CHẶN CỬA TÀI KHOẢN BỊ KHÓA
                    // =======================================================
        ...
        // =======================================================
        // 🔥 XỬ LÝ NHÀ GIAM (LOCKOUT) VÀ NHẬN KHÁNG CÁO
        // =======================================================
        [AllowAnonymous]
        [HttpGet]
        public async Task<IActionResult> Lockout(string email)
        {
            ...
            ViewBag.Reason = profile?.LockReason ?? "Vi phạm nghiêm trọng chính sách của Velvy Skin.";
        ...
        public async Task<IActionResult> SubmitAppeal(string email, string appealReason)
        {
            ...
            if (user != null) {
                ...
                // Tạo Support Ticket đúng chuẩn hệ thống sếp đang dùng
                var ticket = new SupportTicket {
                    UserId = user.Id,
                    UserEmail = user.Email,
                    UserFullName = profile?.FullName ?? "Khách hàng",
                    Subject = "Kháng cáo khóa tài khoản",
                    Content = appealReason, 
                    IssueType = "Kháng cáo", // Tag riêng để Admin dễ nhận biết
                    Status = "Pending",
                ...
                // Tạo kèm TicketMessage để khách và admin chat với nhau
           ...
           if (profile == null)
            {
                ...
                { 
                    ...
                    Allergies = "",
                    Gender = "",
                    LockReason = "" // 🔥 THÊM ĐÚNG 1 DÒNG NÀY VÀO LÀ HẾT LỖI
                };
            ...
            ViewBag.SupportTickets = await _context.SupportTickets
...
ViewBag.RecentOrders = await _context.Orders
        ...
        public async Task<IActionResult> SubmitSupportTicket(SupportTicket ticket, IFormFile attachment)
        {
            ...
            if (VelvySkinWeb.Models.GlobalSettings.AutoReplyComplaint && !string.IsNullOrEmpty(userEmail))
            {
                ...
                string body = $@"
                    <div style='font-family: Arial, sans-serif; padding: 25px; border: 1px solid #e0e0e0; border-radius: 15px; max-width: 600px; margin: 0 auto;'>
                        <h2 style='color: #2ec4b6; text-align: center;'>Xin chào {ticket.UserFullName},</h2>
                        <p>Hệ thống CSKH của Velvy Skin đã tiếp nhận yêu cầu hỗ trợ của bạn với các thông tin sau:</p>
                        <div style='background: #f9f9f9; padding: 15px; border-left: 4px solid #c5a059; margin: 15px 0;'>
                            <p><b>Mã đơn hàng:</b> {ticket.OrderCode ?? "Không có"}</p>
                            <p><b>Loại vấn đề:</b> {ticket.IssueType ?? "Khác"}</p>
                            <p><b>Nội dung chi tiết:</b></p>
                            <p><i>{ticket.Content}</i></p>
                        </div>
                        <p>Đội ngũ chuyên viên của chúng tôi đang tiến hành kiểm tra và sẽ liên hệ/phản hồi lại cho bạn trong thời gian sớm nhất (Thông thường từ 2 - 4 tiếng trong giờ hành chính).</p>
                        <p>Cảm ơn bạn đã kiên nhẫn và đồng hành cùng Velvy Skin!</p>
                        <hr style='border: none; border-top: 1px solid #eee; margin: 20px 0;'>
                        <p style='color: #888; font-size: 12px; text-align: center;'>Đây là email tự động từ hệ thống Velvy Skin. Vui lòng không trả lời email này.</p>
                    </div>";

               
                await VelvySkinWeb.Helpers.EmailHelper.SendEmailAsync(userEmail, subject, body);
            }
        ...
        public async Task<IActionResult> AutoCreateOrderTicket(string orderCode)
        {
            ...
            var existingTicket = await _context.SupportTickets
                .FirstOrDefaultAsync(t => t.UserId == userId && t.OrderCode == orderCode && t.Status != "Đã hoàn thành");

            if (existingTicket != null)
            {
            
                return RedirectToAction("TicketDetail", "Account", new { id = existingTicket.Id });
            }
            ...
            var newTicket = new SupportTicket
            {
                ...
                UserFullName = userProfile?.FullName ?? "Khách hàng",
                OrderCode = orderCode,
                IssueType = "Hỗ trợ đơn hàng",
                Subject = $"Cần hỗ trợ vận chuyển đơn {orderCode}",
                Content = $"Chào Admin, tôi cần hỗ trợ thông tin giao nhận của đơn hàng {orderCode}.",
                CreatedAt = DateTime.Now,
                Status = "Pending"
            };
        ...
        public async Task<IActionResult> CreateProductConsultationTicket(int productId)
        {
            ...
            var existingTicket = await _context.SupportTickets
                .FirstOrDefaultAsync(t => t.UserId == userId && t.Subject == ticketSubject && t.Status != "Đã hoàn thành");
            ...
            var newTicket = new SupportTicket
            {
                UserId = userId,
                UserEmail = User.Identity.Name,
                UserFullName = userProfile?.FullName ?? "Khách hàng Velvy",
   
                IssueType = "Tư vấn làm đẹp", 
                
                Subject = ticketSubject,
                Content = $"Chào chuyên gia Velvy Skin, em cần tư vấn chi tiết hơn về cách dùng và thành phần của sản phẩm '{product.Name}' (Mã SP: {product.Id}) ạ.",
                CreatedAt = DateTime.Now,
                Status = "Pending" 
            };
        ...
        public async Task<IActionResult> ChatDetail(int id)
        {
            ...
            var unreadNoti = await _context.Notifications
                .FirstOrDefaultAsync(n => n.UserId == userId && n.TargetUrl.Contains($"/Account/ChatDetail/{id}") && !n.IsRead);
            if (unreadNoti != null)
            {
